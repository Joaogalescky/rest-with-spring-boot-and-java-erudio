...
public class AuthControllerYamlTest extends AbstractIntegrationTest {
	...
	public void testSignin() throws JsonMappingException, JsonProcessingException {
		...
		tokenVO = given().spec(specification)
				...
				.contentType(TestConfigs.CONTENT_TYPE_YML).body(user, objectMapper).when().post().then().statusCode(200)
	...
	public void testRefresh() throws JsonMappingException, JsonProcessingException {
		...
		var newTokenVO = given()
				.config(RestAssuredConfig.config()
						.encoderConfig(EncoderConfig.encoderConfig().encodeContentTypeAs(TestConfigs.CONTENT_TYPE_YML,
								ContentType.TEXT)))
				.accept(TestConfigs.CONTENT_TYPE_YML).basePath("/auth/refresh").port(TestConfigs.SERVER_PORT)
